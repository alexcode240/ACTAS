...
class WordController
{
    ...
    public function ctrGenerateWord()
    {
        ...
        $hecho1 = "1.	Los Servidores Públicos con adscripción al Órgano Interno de Control, a la Subdirección de Patrimonio Municipal, a la Segunda Sindicatura y al " . $respuesta["FCAREA"] . ", se constituyen el día, hora y lugar referido en el párrafo primero de la presente Acta Circunstanciada, esto en atención al oficio con número de folio " . $respuesta["FCOFICIO"] . ", de fecha " . $fechaOficio["dia"] . " de " . $fechaOficio["mes"] . " de " . $fechaOficio["anio"] . ", suscrito por el C.P. Ricardo Contreras Velázquez, Contralor Interno Municipal, el cual fue notificado el día " . $fechaNotificacion["dia"] . " de " . $fechaNotificacion["mes"] . " de " . $fechaNotificacion["anio"] . ", al C. " . $respuesta["FCENLACE"] . " en su carácter de Enlace Administrativo del " . $respuesta["FCAREA"] . " de Tlalnepantla de Baz, Estado de México, administración 2022-2024, a fin de comunicarle que el PRIMER LEVANTAMIENTO FÍSICO DE BIENES MUEBLES DE 2022, sería el " . $fechaLevantamiento["dia"] . " de " . $fechaLevantamiento["mes"] . " del " . $fechaLevantamiento["anio"] . " al " . $fechaFinLevantamiento["dia"] . " de " . $fechaFinLevantamiento["mes"] . " de " . $fechaFinLevantamiento["anio"] . ", a partir de las " . $fechaLevantamiento["hora"] . " horas, para verificar la existencia física de los bienes muebles en uso y custodia del Instituto, con base en el LISTADO DE BIENES MUEBLES DEL PRIMER LEVANTAMIENTO FÍSICO 2022, del INSTITUTO MUNICIPAL DE LA JUVENTUD, expedido por la Subdirección de Patrimonio Municipal, dependiente de la Secretaría del Ayuntamiento.";
        ...
        $section->addText("Folio: " . $respuesta["FCFOLIO"], 'ptFont', 'ptStyle');
        ...
        $hecho3 = "Acto seguido, el C. ".$respuesta["FCENLACE"].", Enlace Administrativo del ".$respuesta["FCAREA"].", manifiesta: el personal de la Subdirección de Patrimonio Municipal, como de la Segunda Sindicatura y de la Contraloría Interna Municipal, asistieron al ".$respuesta["FCAREA"].", el ".$fechaLevantamiento["dia"]." de ".$fechaLevantamiento["mes"]." de ".$fechaLevantamiento["anio"].", para la verificación física de los bienes muebles en uso y custodia del Instituto en comento.";
        
        $section->addText(htmlspecialchars($hecho3, ENT_COMPAT, 'UTF-8') , 'pFont', 'pStyle');

        //$section->addTextBreak();

        $textoTotales = "En uso de la palabra, la C. ".$respuesta["FCPATRIMONIO"].", representante de la Subdirección de Patrimonio Municipal, manifiesta que el PRIMER LEVANTAMIENTO FISICO DE BIENES MUEBLES DE 2022, del ".$respuesta["FCAREA"].", se realizó con el documento denominado: LISTADO DE BIENES MUEBLES DEL PRIMER LEVANTAMIENTO FÍSICO 2022, del ".$respuesta["FCAREA"].", emitido por la Subdirección de Patrimonio Municipal, mismo que se detalla en el “ANEXO A” y se integra de la forma siguiente:";
        ...
        $textoPreTotales = "Posteriormente, en uso de la palabra el C. ".$respuesta["FCCONTRALOR"].", representante del Órgano Interno de Control, manifiesta que una vez conciliada la información con el personal habilitado de la Subdirección de Patrimonio Municipal, de la Segunda Sindicatura y del ".$respuesta["FCAREA"].", se obtuvo como resultado del PRIMER LEVANTAMIENTO FÍSICO DE BIENES MUEBLES DE 2022, lo siguiente:";
